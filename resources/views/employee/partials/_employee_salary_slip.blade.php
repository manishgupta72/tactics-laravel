...
<body>
    ...
    <table>
        ...
        @if(isset($dynamicEarnings) && isset($dynamicDeductions) && ($dynamicEarnings->count() > 0 || $dynamicDeductions->count() > 0))
            <tr class="bg-light-gray">
                <th colspan="4">Additional Components</th>
            </tr>
            
            <!-- Display dynamic earnings and deductions side by side -->
            @php $maxCount = max($dynamicEarnings->count(), $dynamicDeductions->count()); @endphp
            
            @for($i = 0; $i < $maxCount; $i++)
                <tr>
                    @if(isset($dynamicEarnings[$i]))
                        <td><strong>{{ $dynamicEarnings[$i]->component_name }}</strong></td>
                        <td>{{ $dynamicEarnings[$i]->amount ?: 0 }}</td>
                    @else
                        <td></td>
                        <td></td>
                    @endif
                    
                    @if(isset($dynamicDeductions[$i]))
                        <td><strong> {{ $dynamicDeductions[$i]->component_name }}</strong></td>
                        <td>{{ $dynamicDeductions[$i]->amount ?: 0 }}</td>
                    @else
                        <td></td>
                        <td></td>
                    @endif
